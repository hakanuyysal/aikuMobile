...
const CARD_WIDTH = metrics.getWidthPercentage(75); // Kart genişliğini biraz azalttım
const SPACING = metrics.spacing.lg; // Spacing'i artırdım
const ITEM_TOTAL_WIDTH = CARD_WIDTH + SPACING;
const SIDECARD_OFFSET = metrics.spacing.lg; // Offset'i azalttım, böylece diğer kartlar görünecek
...
const PlanCard: React.FC<PlanProps> = ({
    ...
    const initializePlan = async () => {
      try {
        ...
        if (packages.length === 0) {
          console.error(
            '❌ RevenueCat offerings empty! Check offerings in dashboard.',
          );
        } else {
        ...
        try {
          const subscriptionsResponse =
            await RevenueCatService.getAllSubscriptions();
          if (
            ...
            const activeSubscriptions = allSubscriptions.filter((sub: any) => {
              ...
              if (sub.isActive) {
                ...
                console.log(
                  `   📅 End date: ${endDate}, Now: ${now}, Not expired: ${isNotExpired}`,
                );
                if (isNotExpired) {
            ...
            if (currentPlan) {
              ...
              console.log(
                `✅ Active subscription found: ${planName} ${planPeriod}`,
              );
              console.log(
                `   Final status: ${currentPlan.status}, isActive: ${currentPlan.isActive}`,
              );
              ...
              console.log(
                `❌ No active subscription found: ${planName} ${planPeriod}`,
              );
              console.log(
                `   Available subscriptions for this plan:`,
                activeSubscriptions,
              );
            }
          }
  ...
  const scale = scrollX.interpolate({
    inputRange,
    outputRange: [0.9, 1.05, 0.9], // Scale değerlerini daha yumuşak yaptım
  });

  const opacity = scrollX.interpolate({
    inputRange,
    outputRange: [0.8, 1, 0.8], // Opacity değerlerini artırdım
  });

  const rotateY = scrollX.interpolate({
    inputRange,
    outputRange: ['15deg', '0deg', '-15deg'], // Rotasyon değerlerini azalttım
  });

  const translateY = scrollX.interpolate({
    inputRange,
    outputRange: [20, 0, 20], // TranslateY değerlerini azalttım
  });

  const translateX = scrollX.interpolate({
    inputRange,
    outputRange: [-10, 0, 10], // TranslateX değerlerini azalttım
  });
    ...
    <Animated.View
      ...
      <View style={{flex: 1, paddingBottom: 64}}>
        ...
        {hasActiveSubscription ? (
          ...
          <>
            <Text style={styles.subscriptionType}>Active Subscription</Text>
            <Text style={styles.price}>
              ${currentSubscription?.amount || price}
              <Text style={styles.period}>
                /{currentSubscription?.period || (isYearly ? 'year' : 'month')}
              </Text>
            </Text>
            <Text style={styles.activeStatus}>
              {currentSubscription?.status === 'active'
                ? '✅ Currently Active'
                : '⚠️ Cancelled but Active Until Expiry'}
            </Text>
